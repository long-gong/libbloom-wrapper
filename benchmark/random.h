...
static inline void remove_duplications(std::vector<T>& result, RNG& genrand) {
  // remove duplications
  std::sort(result.begin(), result.end());
  auto it = std::unique(result.begin(), result.end());
  result.resize(std::distance(result.begin(), it));
  std::shuffle(result.begin(), result.end(), genrand);
}
